    ...
    .then(function(result) {
      ...
      if(regex) {
        if(_.isFunction(regex)) { // check as function
          return regex(html) ? true : Promise.reject('data doesn\'t match provided function');
        } else if(_.isRegExp(regex)) { // check as regex
          return regex.test(html) ? true : Promise.reject('data doesn\'t match provided regex');
        } else { // check as string
          return html.indexOf(regex) != -1 ? true : Promise.reject('data doesn\'t contain provided string');
        }
      } else {
  ...
  var promises = [ // test for http and https support simultaneously
   ...
   return Promise.all(promises)
    .spread(function(resHttp) {
      ...
      //} else { 
        // http proxy was rejected, don't check https then
...
module.exports = function(options) {
  var onlyProxyIP = options.proxyIP.split("@").pop();
  var country = geoip.lookupCountry(onlyProxyIP);
